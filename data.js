...
for(i in products){
    ...
    var gridbox = document.getElementsByClassName("product-container")[0];
    var cardprice = document.createElement("h2");
    cardimg.src=products[i].thumbnail;
    ...
    cardtitle.innerHTML=` ${products[j].title} `;
    carddesc.innerHTML=` ${products[j].description}`;
    cardprice.innerHTML=` Rs ${products[j].price}`;
    c++;
    j++;
}
